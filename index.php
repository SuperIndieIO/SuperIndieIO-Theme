    ...
    <main>
        ...
        <?php $SFS = 0 ?>
            ...
            <?php $thumb = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'post-small' ); ?>
             <a href='<?php echo get_the_permalink(); ?>'>
            <div class='SI-SemiFeatured <?php if ( $SFS % 2 !== 0 ) {echo 'grid-a'; $secondary = $post;} else {echo 'grid-b'; $tertiary = $post;} ?> img-background' style='background-image: linear-gradient(to bottom, rgba(0, 0, 0, .25) 0%, rgba(0, 0, 0, .25) 50%, rgba(0, 0, 0, 1) 100%), url("<?php echo $thumb[0] ?>'>
        ...
        <div style='margin: 0 auto 16px auto; max-width: 970px; max-height: 250px; width: 100%; height: 250px; background-color: #212121;'></div>
